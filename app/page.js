...
export default function Home() {
  ...
  const logoutTodoist = async () => {
    if (!confirm("Todoistとの連携を解除しますか？")) return;
    setTodoistLoggingOut(true);
    try {
      await fetch("/api/todoist/logout", { method: "POST" });
      setTodoistConnected(false);
